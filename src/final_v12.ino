...
int ext_wall_dist = 32;
int mid_wall_dist = 64;
...
void loop() {
  ...
  switch (loop_case) {
    ...
    case TURN_TO_CENTER_CASE: {
      if (abs(gz - (current_angle + center_angle)) > 5) {
        pid_error_gyro = (gz - (current_angle + center_angle)) * kp_gyro + integral * ki_gyro - (pid_last_error_gyro - pid_error_gyro) * kd_gyro;
        integral_gyro += pid_error_gyro;
        pid_last_error_gyro = pid_error_gyro;
        ...
        if (turns == 12)
          ...
          if ((cube.color == 0 || cube.w * cube.h < 500)) { 
            file_println("No cube: ," + String(cube.color) + ", " + String(cube.w * cube.h));
            flag_straight = 1;
            flag = 0;
            curr_angle = gz;
            last_cube_color = cube.color;
